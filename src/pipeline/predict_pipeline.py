...
class PredictionPipeline:
    ...
    def get_predicted_dataframe(self, input_dataframe_path:pd.DataFrame):
        ...
        try:
            ...
            # Log that the prediction process is starting
            logging.info(f"Starting predictions for input file: {input_dataframe_path}")

            # Get predictions from the model (assuming self.predict method is defined)
            predictions = self.predict(input_dataframe)

            # Add the predicted values as a new column in the input dataframe
